  ...
  return (
    <div className='flex items-center justify-center min-h-screen bg-gray-50'>
        ...
        <form onSubmit={handleSubmit} className="sm:w-[350px] w-full text-center border border-gray-300/60 rounded-2xl px-8 bg-white">
                ...
                {state !== "login" && (
                    <div className="flex items-center mt-6 w-full bg-white border border-gray-300/80 h-12 rounded-full overflow-hidden px-4 gap-2 focus-within:border-green-500 focus-within:border-2 transition-all">
                        <User2Icon size={16} color='#6B7280' className="flex-shrink-0" />
                        <input type="text" name="name" placeholder="Ad" className="border-none outline-none ring-0 w-full text-xs" value={formData.name} onChange={handleChange} required />
                    </div>
                )}
                <div className="flex items-center w-full mt-4 bg-white border border-gray-300/80 h-12 rounded-full overflow-hidden px-4 gap-2 focus-within:border-green-500 focus-within:border-2 transition-all">
                   <Mail size={13} color='#6B7280' className="flex-shrink-0" />
                    <input type="email" name="email" placeholder="Email" className="border-none outline-none ring-0 w-full text-xs" value={formData.email} onChange={handleChange} required />
                </div>
                <div className="flex items-center mt-4 w-full bg-white border border-gray-300/80 h-12 rounded-full overflow-hidden px-4 gap-2 focus-within:border-green-500 focus-within:border-2 transition-all">
                    <Lock size={13} color='#6B7280' className="flex-shrink-0" />
                    <input type="password" name="password" placeholder="Şifrə" className="border-none outline-none ring-0 w-full text-xs" value={formData.password} onChange={handleChange} required />
                </div>
                {state === "login" && (
        ...
        <form onSubmit={handleForgotPassword} className="sm:w-[350px] w-full text-center border border-gray-300/60 rounded-2xl px-8 bg-white">
            ...
            <div className="flex items-center w-full mt-4 bg-white border border-gray-300/80 h-12 rounded-full overflow-hidden px-4 gap-2 focus-within:border-green-500 focus-within:border-2 transition-all">
               <Mail size={13} color='#6B7280' className="flex-shrink-0" />
                <input type="email" placeholder="Email" className="border-none outline-none ring-0 w-full text-xs" value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} required />
            </div>
